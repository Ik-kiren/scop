...
Matrix4 Matrix4::operator*(Matrix4 rhs)
{
   ...
   for (int i = 0; i < 4; i++)
   {
      for (int j = 0; j < 4; j++)
      {
         mat[i][j] = matrix[i][0] * rhs[0][j] + matrix[i][1] * rhs[1][j] + matrix[i][2] * rhs[2][j] + matrix[i][3] * rhs[3][j];
      }
   }
   return mat;
}
...
Vector4 Matrix4::operator*(Vector4 rhs)
{
   ...
   for (int i = 0; i < 4; i++)
   {
      vec[i] = matrix[i][0] * rhs[0] + matrix[i][1] * rhs[1] + matrix[i][2] * rhs[2] + matrix[i][3] * rhs[3];
   }
   return vec;
}
